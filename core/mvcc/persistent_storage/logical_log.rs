...
const TX_EXT_HEADER_SIZE: usize =
...
impl LogicalLog {
    ...
    fn serialize_and_pwrite_tx(
        ...
        // payload_size is only known after serializing all ops. We reserve the selected header
        ...
        #[cfg(feature = "conn_raw_api")]
        let extension_block = if tx.portable_changes.is_empty() {
            Vec::new()
        } else {
            let portable_changes = encode_portable_change_payload_with_stable_end_offset(
                self.offset,
                self.write_buf.len() + EXTENSION_RECORD_HEADER_SIZE,
                tx.tx_timestamp,
                &tx.portable_changes,
            )?;
...
mod tests {
    ...
    fn test_next_portable_change_frame_returns_empty_and_nonempty_lml3_frames() {
        ...
        let sync_tx = crate::mvcc::database::LogRecord {
            ...
            header: None,
            #[cfg(feature = "conn_raw_api")]
            portable_changes: encoded_empty_logical_op,
        };
